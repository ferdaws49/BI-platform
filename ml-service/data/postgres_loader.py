...
def load_ca_data() -> pd.DataFrame:
    """
    CA mensuel depuis la table finances (pas le DWH dim_temps / fact_finance).

    Types Finance :
      paiement, depense_formateur, depense_logistique, impaye
    """
    engine = get_engine()
    ...
    query = """
        SELECT
            ...
            COALESCE(SUM(f.montant) FILTER (WHERE f.type = 'impaye'), 0)
                                                                             AS total_impaye,
            COUNT(DISTINCT f."sessionId")                                    AS nb_sessions,
            (
                SELECT COUNT(DISTINCT sa."apprenantId")
                FROM sessions_apprenants sa
                JOIN sessions s ON s.id = sa."sessionId"
                WHERE EXTRACT(YEAR  FROM s.date) = EXTRACT(YEAR  FROM f.date)
                  AND EXTRACT(MONTH FROM s.date) = EXTRACT(MONTH FROM f.date)
            )                                                                AS total_inscrits
        FROM finances f
    ...
    # Ajouter colonne 'mois' attendue par build_features()
    if "mois_num" in df.columns and "mois" not in df.columns:
        df["mois"] = df["mois_num"].astype(int)

    assert len(df) >= 15, f"Pas assez de mois historiques ({len(df)}). Minimum 15."
    print(f"[load_ca_data] ✅ {len(df)} mois chargés depuis PostgreSQL")
    return df


def load_sessions_data() -> pd.DataFrame:
    """
    Features financières par session + label est_deficitaire.
    Label : 1 si (cout_formateur + cout_logistique) > revenus paiements.
    """
    engine = get_engine()
    ...
    query = """
        SELECT
            ...
            COALESCE(s.cout_formateur,  0)                                   AS cout_formateur,
            COALESCE(s.cout_logistique, 0)                                   AS cout_logistique,
            COALESCE(
                SUM(f.montant) FILTER (WHERE f.type = 'paiement'), 0
            )                                                                AS montant_inscriptions,
            CASE
                WHEN (COALESCE(s.cout_formateur, 0) + COALESCE(s.cout_logistique, 0))
                     > COALESCE(SUM(f.montant) FILTER (WHERE f.type = 'paiement'), 0)
                THEN 1 ELSE 0
        ...
        GROUP BY s.id, s.date, s.capacite, s.type, s.cout_formateur, s.cout_logistique
    ...
    # Garantir présence de 'duree_jours' attendue par train_deficit
    if "duree_jours" not in df.columns:
        df["duree_jours"] = 1

    # Normaliser type de 'mois' en int si nécessaire
