"""
models/forecast.py - Inference prevision inscriptions (Option B + fallback)
"""
...
class ForecastRegistry:
    ...
    def predict_next_months(self, df: pd.DataFrame, periodes: int) -> list:
        if self.model_name == "Prophet":
            try:
                ...
                result = self._predict_prophet(periodes, df=df)
                self.last_model_used = "Prophet"
                logger.info("[Forecast] Prediction via Prophet (%s mois)", periodes)
                return result
            except Exception as e:
                logger.warning(
                    "[Forecast] Prophet indisponible ou erreur predict : %s -> fallback LinearTrend", e,
                )
                print(f"[Forecast] Fallback LinearTrend : {e}")

        return self._predict_with_linear_fallback(periodes, reason="primary_or_fallback", df=df)
    ...
    def _predict_prophet(self, periodes: int, df: pd.DataFrame = None) -> list:
        last_date = pd.to_datetime(df["ds"].iloc[-1]) if df is not None and len(df) >= 1 else None

        future = self.model.make_future_dataframe(periods=periodes, freq="MS")
        forecast = self.model.predict(future)

        if last_date is not None:
            future_rows = forecast[forecast["ds"] > last_date].head(periodes)
        else:
    ...
    def _predict_with_linear_fallback(self, periodes: int, reason: str, df: pd.DataFrame = None) -> list:
        ...
        # last_date من الـ request — n من الـ training
        ref = df if df is not None and len(df) >= 1 else self.last_data.copy()
        ...
        for i in range(1, periodes + 1):
            ...
            std = max(1, int(np.std(self.last_data["y"].values) * 0.5))
