...
interface NotifData {
  ...
  performanceNotifs: boolean;
}

// ─── Main Component ──────────────────────────────────────────────────────────
export default function SettingsPage() {
  const { t: fullT } = useDirecteurContext();
  ...
  const [notifs, setNotifs] = useState<NotifData>({
    ...
    weeklyReport: true,
    systemUpdates: true,
    performanceNotifs: true,
  });

  // ── Load localStorage on mount ──
  useEffect(() => {
    const savedRefresh =
      localStorage.getItem("dataRefresh-directeur") ?? "5min";
    const savedNotifs = localStorage.getItem("notifications-directeur");
  ...
  const handleSaveNotifs = () => {
    localStorage.setItem("notifications-directeur", JSON.stringify(notifs));
    setNotifSaved(true);
    setTimeout(() => setNotifSaved(false), 2000);
  };

  const handleSavePrefs = () => {
    localStorage.setItem("dataRefresh-directeur", dataRefresh);
    setPrefSaved(true);
